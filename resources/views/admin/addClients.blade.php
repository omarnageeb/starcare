...
<!--
                ...
                @endif
-->
            <form class="marg" method="post" action="{{route('addClientsPost')}}" enctype="multipart/form-data">
                 @csrf
                    <div class="mb-3">
            ...
            @foreach($clients as $addClients)
                <tr>
                    <td><img  style="width: 90px; height: 90px;" src="{{asset('images/clients/'.$addClients->photo)}}"></td>
                    <td>{{$addClients -> description_en}}</td>
                    <td>{{$addClients -> description_ar}}</td>
                     <td>
                          <a href="{{route('client.edit',$addClients -> id)}}" class="btn btn-success"> {{__('messages.edit')}}</a>
                     </td>
                     <td>
                          <a href="{{route('client.delete',$addClients -> id)}}" class="btn btn-danger"> {{__('messages.delete')}}</a>
                     </td>
                </tr>
            @endforeach
